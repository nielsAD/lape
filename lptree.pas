...
type
  ...
  public
    ...
    function getTempVar(var Offset: Integer; out v: TResVar; Lock: Integer = 1): Boolean; virtual;
  ...
  public
    ...
    function getBoolVar(var Offset: Integer; out ConditionVar: TResVar): Boolean; virtual;
  end;
  ...
  public
    function getTempVar(var Offset: Integer; out v: TResVar; Lock: Integer = 1): Boolean; override;
...
function TLapeTree_Base.getTempVar(var Offset: Integer; out v: TResVar; Lock: Integer = 1): Boolean;
begin
  v := Compile(Offset);
...
function TLapeTree_ExprBase.getBoolVar(var Offset: Integer; out ConditionVar: TResVar): Boolean;
var
...
function TLapeTree_DestExprBase.getTempVar(var Offset: Integer; out v: TResVar; Lock: Integer = 1): Boolean;
begin
  Dest := VarResVar;
  Result := inherited;
end;
  ...
  begin
    ...
    for i := FValues.Count - 1 downto 0 do
      ...
      try
        ...
        begin
          Left := TLapeTree_ResVar.Create(Result, FValues[i]);
          Right := TLapeTree_Integer.Create(i + TLapeType_StaticArray(ToType).Range.Lo, FValues[i]);
        end;
    ...
    for i := FValues.Count - 1 downto 0 do
      if (FValues[i] is TLapeTree_ExprBase) then
        ...
        try
          ...
          begin
            Left := TLapeTree_ResVar.Create(Result, FValues[i]);
            Right := TLapeTree_Field.Create(TLapeType_Record(ToType).FieldMap.Key[i], FCompiler, @FValues[i]._DocPos);
          end;
          Right := FValues[i] as TLapeTree_ExprBase;
          Compile(Offset);
        finally
...
begin
  if (FIdent is TLapeTree_VarType) then
    Result := (FParams.Count = 1) and (not isEmpty(FParams[0])) and FParams[0].isConstant()
  else
    Result := False;
end;
...
var
      ...
      try
        ...
        if (ParamVars[i].VarPos.MemPos = NullResVar.VarPos.MemPos) then
          if (Params[i].ParType in Lape_RefParams) then
            FParams[i].getTempVar(Offset, ParamVars[i], 0)
          else
  ...
  begin
    ...
    try
      addParam(TLapeTree_ResVar.Create(IdentVar, Self));
  ...
  if (_Dispose <> nil) then
    ...
    try
      ...
      begin
        ...
        TLapeTree_Operator(Params[0]).Left := TLapeTree_ResVar.Create(Param, Self.FParams[0]);
      end
      else
...
begin
  ...
  if (not FParams[0].getTempVar(Offset, Param, BigLock)) or (not Param.HasType()) or
  ...
  if (not FParams[1].getTempVar(Offset, Len, BigLock)) or (ArrayType = nil) or
  ...
  try
    ...
    if (VarParam.VarType.BaseType = ltPointer) then
      ...
      try
        ...
        Right := TLapeTree_ResVar.Create(CountParam, Self);
        Result := Compile(Offset);
        Self.Dest := Dest;
      finally
        Free();
...
begin
  ...
  try
    ...
    Succ.Left := TLapeTree_ResVar.Create(Result, OldVarParam);
    Succ.Right := TLapeTree_InternalMethod_Succ.Create(Self);
    TLapeTree_InternalMethod_Succ(Succ.Right).addParam(TLapeTree_ResVar.Create(Result, OldVarParam));
    if (OldCountParam <> nil) then
...
begin
  ...
  try
    ...
    Pred.Left := TLapeTree_ResVar.Create(Result, OldVarParam);
    Pred.Right := TLapeTree_InternalMethod_Pred.Create(Self);
    TLapeTree_InternalMethod_Pred(Pred.Right).addParam(TLapeTree_ResVar.Create(Result, OldVarParam));
    if (OldCountParam <> nil) then
...
begin
  Result := Self;
  if (not isEmpty(Self)) and (OperatorType = op_Deref) and (FLeft <> nil) and (ExpectType <> nil) then
  begin
    Typ := FLeft.resType();
...
begin
  ...
  begin
    ...
    if (FOperatorType = op_IN) then
      ...
      TLapeTree_Base(FLeft) := TLapeTree_If.Create(Self);
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        Result.isConstant := False;
        TLapeTree_Operator(Body).Left := TLapeTree_ResVar.Create(Result, Self);
        TLapeTree_Operator(ElseBody).Left := TLapeTree_ResVar.Create(Result, Self);
      end;

      if (Result.VarPos.MemPos = mpVar) and (Result.VarPos.StackVar is TLapeStackTempVar) then
        TLapeStackTempVar(Result.VarPos.StackVar).IncLock();
      Result := Compile(Offset);
    end;
  end;
...
begin
  ...
  for i := 0 to FWithList.Count - 1 do
    ...
    begin
      if (not FWithList[i].getTempVar(Offset, ResVarList[i], BigLock)) or
...
begin
  ...
  if (not FCondition.getBoolVar(Offset, ConditionVar)) then
...
begin
  ...
  CheckField := nil;
  opOR := nil;
  if (not FCondition.getTempVar(Offset, ConditionVar, BigLock)) then
    LapeException(lpeInvalidCondition, DocPos);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        LapeException(lpeInvalidEvaluation, DocPos);
      CheckField := CheckField.FoldConstants() as TLapeTree_Operator;
...
begin
  ...
  try
    IncDec.addParam(TLapeTree_ResVar.Create(TLapeTree_ResVar(TLapeTree_Operator(FCondition).Left).ResVar, FCondition));
    if (FStep <> nil) then
    begin
...
begin
  ...
  try
    if (not FLimit.getTempVar(Offset, Lim, BigLock)) or (not Lim.HasType()) or (Lim.VarType.BaseIntType = ltUnknown) then
